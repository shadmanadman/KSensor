...
/**
 * No-op MotionPlugin for the web (Wasm) target. KSensor has no web (Wasm) sensor
 * backend yet, so observations emit nothing and state values are placeholders.
 */
class WasmMotionPlugin : MotionPlugin {
...
actual fun createMotionPlugin(): MotionPlugin = WasmMotionPlugin()
